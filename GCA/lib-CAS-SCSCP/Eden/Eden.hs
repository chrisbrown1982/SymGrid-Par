...
module Eden(
	...
	---------------------------------
	, Lift(..), deLift, createProcess -- deprecated legacy code for Eden 5
    )
  where
...
#ifndef __SIMUL__
import qualified ParPrim(noPe,selfPe)
import ParPrim hiding(noPe,selfPe)
#else
import qualified ParPrimSim as ParPrim
import ParPrimSim hiding(noPe,selfPe)
#endif
